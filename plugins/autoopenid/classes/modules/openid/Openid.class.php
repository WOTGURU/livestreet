 ...
 *
 */
class PluginAutoopenid_ModuleOpenid extends Module {
	...
	public function Init() {
		 ...
		 */
		if (Config::Get('plugin.autoopenid.buggy_gmp')) {
		 ...
		 */
		$this->oConsumer=new Auth_OpenID_Consumer(new Auth_OpenID_FileStore(Config::Get('plugin.autoopenid.file_store')));
	}
	/**
	...
	public function Login($sOpenId,$sPath) {
		...
		if ($auth_request->shouldSendRedirect()) {			
			...
			$form_html = str_replace('method="post"','method="get"',$form_html);
	...
	public function Verify($sPath) {
		...
		if ($response->status == Auth_OpenID_CANCEL) {			
			 ...
			 */
			$aReturn['msg']=$this->Lang_Get('plugin.autoopenid.openid_result_cancel');
			 ...
			 */
			$aReturn['msg']=$this->Lang_Get('plugin.autoopenid.openid_result_success');
			$aReturn['status']=true;
	...
	protected function getTrustRoot() {
		$aVar=explode(':', $_SERVER['HTTP_HOST']);
		return sprintf("%s://%s:%s%s/",
			$this->getScheme(), array_shift($aVar),
			$_SERVER['SERVER_PORT'],
		'');
	}
	/**
	...
	/**
	 * Обновляет запись OpenId
	 *
	 * @param PluginAutoopenid_ModuleOpenid_EntityOpenid $oOpenId
	 * @return unknown
	 */
	public function UpdateOpenId(PluginAutoopenid_ModuleOpenid_EntityOpenid $oOpenId) {
		return $this->oMapper->UpdateOpenId($oOpenId);
	}	
