 ...
 *
 */
class PluginAutoopenid_ModuleOauth extends Module {
	...
	public function LoginTwitter($sPath) {		
		$OAuth = new TwitterOAuth(Config::Get('plugin.autoopenid.twitter.token'), Config::Get('plugin.autoopenid.twitter.token_secret'));		
	...
	public function VerifyTwitter() {		
		...
		$OAuth = new TwitterOAuth(Config::Get('plugin.autoopenid.twitter.token'), Config::Get('plugin.autoopenid.twitter.token_secret'), $_SESSION['twitter_oauth_token'], $_SESSION['twitter_oauth_token_secret']);		
	...
	protected function CheckTwitter() {
		...
		$OAuth = new TwitterOAuth(Config::Get('plugin.autoopenid.twitter.token'), Config::Get('plugin.autoopenid.twitter.token_secret'), $aAccessToken['oauth_token'], $aAccessToken['oauth_token_secret']);
		if ($OAuth) {
	...
	public function GetTokens() {
		return array("key"=>$this->OAuthTwitter->token->key,"secret"=>$this->OAuthTwitter->token->secret); 
	}		
}
?>