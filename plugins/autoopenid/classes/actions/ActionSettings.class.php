 ...
 *
 */
class PluginAutoopenid_ActionSettings extends ActionPlugin {		
	/**
	 * Какое меню активно
	 *
	 * @var unknown_type
	...
	protected function EventSettings() {
		 ...
		 */
		$aOpenId=$this->PluginAutoopenid_Openid_GetOpenIdByUser($this->oUserCurrent->getId());
	...
	protected function EventAjaxDeleteOpenId() {
		...
		if(!($oOpenId=$this->PluginAutoopenid_Openid_GetOpenId($sOpenId))) {
		...
		if (count($aOpenId)==1 and !$this->oUserCurrent->getMail()) {
			$this->Message_AddErrorSingle($this->Lang_Get('plugin.autoopenid.openid_menu_settings_delete_last_error'),$this->Lang_Get('error'));
			return;
		}
		/**
		 * Удаляем
		 */
		$this->PluginAutoopenid_Openid_DeleteOpenId($sOpenId);
		$this->Message_AddNoticeSingle($this->Lang_Get('plugin.autoopenid.openid_menu_settings_delete_ok'),$this->Lang_Get('attention'));		
	}
	/**
