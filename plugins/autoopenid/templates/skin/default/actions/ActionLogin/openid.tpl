...
<div class="openid-block">
	<h1>{$aLang.plugin.autoopenid.openid_enter_title} <img src="{$sTemplateWebPathPlugin}img/openid.png" alt="openid" class="openid-img" title="{$aLang.plugin.autoopenid.openid}" alt="{$aLang.plugin.autoopenid.openid}"/></h1>
	...
	<form method="post" action="{router page='login'}openid/enter/" name="fopenid" id="openid_form">
		<div style="overflow: hidden; zoom: 1;">
			...
			<a href="#" class="openid-login" onclick="getEl('openid_form').submit(); return false;"><span>{$aLang.plugin.autoopenid.openid_enter}</span></a>
		</div>
		
		<div class="openid-services">
<!--			<p>{$aLang.plugin.autoopenid.openid_choose_service}</p>-->
...
var iFbAppId='{cfg name='plugin.autoopenid.fb.id'}';
